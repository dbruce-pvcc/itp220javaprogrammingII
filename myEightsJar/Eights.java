...
public class Eights {
    ...
    private PlayerEights one;
    private PlayerEights two;
    ...
    public Eights() {
        ...
        one = new PlayerEights("Rosy");
        deck.deal(one.getHand(), 5);
        two = new PlayerEights("Lemon");
    ...
    public void reshuffle() {
        // save the top card
        Card prevdiscard = discardPile.popCard();

        // move the rest of the cards
        discardPile.dealAll(drawPile);

        // put the top card back
        discardPile.addCard(prevdiscard);
     ...
     * Switches players.
     */
    public PlayerEights nextPlayer(PlayerEights current) {
        if (current == one) {
            return two;
        } else {
            return one;
     ...
     */
    public void takeTurn(PlayerEights player) {
        Card prevdiscard = discardPile.lastCard();
        Card next = player.play(this, prevdiscard);
